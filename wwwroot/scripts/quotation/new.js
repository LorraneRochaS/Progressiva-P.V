!function ($) {
    ...
    $('#IsApp, #VehiclePlate, #VehicleBrandId, #VehicleYear, #VehicleModelId').change(validDivVehicle);
    function validDivVehicle() {
     var isApp = $('#IsApp').prop('checked');
     var vehiclePlate = $('#VehiclePlate').val().trim();
     var vehicleBrandId = $('#VehicleBrandId').val();
     var vehicleYear = $('#VehicleYear').val();
     var vehicleModelId = $('#VehicleModelIdr').val();

        if (isApp && vehiclePlate != '' && vehicleModelId != '' && vehicleBrandId > 0 && vehicleYear > 0) {
        document.querySelector('#associated-next').disabled = 0;
         } else {
         document.querySelector('#associated-next').disabled = 1;
         }
